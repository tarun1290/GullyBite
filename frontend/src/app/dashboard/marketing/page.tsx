...
// Absorbed from the deleted /dashboard/marketing hub — these are the
// existing shared section components (only the hub consumed them); now
// surfaced as tabs here. Relative depth is unchanged by the later
// campaigns/ → marketing/ folder rename, so this path stays valid.
...
// Absorbed standalone pages (Phase-2 consolidation): the former
// /dashboard/{customers,loyalty,referrals,dine-in} routes, moved here
// verbatim as co-located tab components (captain-listing merged inside
// ReferralsTab). Same directory depth → their internal relative imports
// are unchanged.
...
type TabKey = 'manual' | 'journeys' | 'customers' | 'loyalty' | 'coupons' | 'referrals' | 'dinein' | 'messages';
// Validation allowlist for the ?tab= query param. Kept in lockstep with
// the TabKey union above (hard-coded so this file doesn't pull a types
// module just for deep-link validation).
const VALID_TAB_KEYS: readonly TabKey[] = [
  'manual', 'journeys', 'customers', 'loyalty', 'coupons', 'referrals', 'dinein', 'messages',
];
...
export default function MarketingPage() {
  ...
  // ?tab= deep-linking — initialise from the URL so the right tab is
  // active on first paint (no flash of the default). Missing/invalid →
  // 'manual'. This page is the only one with ?tab= sync.
  const tabFromUrl = ((): TabKey => {
    const t = searchParams.get('tab');
    return t && (VALID_TAB_KEYS as readonly string[]).includes(t) ? (t as TabKey) : 'manual';
  })();
  ...
  // Keep local tab state in sync when ?tab= changes via browser
  // back/forward or an external deep-link (e.g. captain-nav's
  // GullyBite Referrals → /dashboard/marketing?tab=referrals).
  useEffect(() => {
    const t = searchParams.get('tab');
    const next: TabKey = t && (VALID_TAB_KEYS as readonly string[]).includes(t) ? (t as TabKey) : 'manual';
    setTab((prev) => (prev === next ? prev : next));
  }, [searchParams]);

  // Tab click → update state + keep URL in sync. Default tab ('manual')
  // drops the query entirely; others write ?tab=<key>. router.replace
  // (not push) avoids spamming history; scroll:false holds viewport.
  const selectTab = (next: TabKey) => {
    setTab(next);
    router.replace(next === 'manual' ? pathname : `${pathname}?tab=${next}`, { scroll: false });
  };
