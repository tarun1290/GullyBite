...
import { bulkUploadImages, getImageStats } from '../../../api/restaurant.js';

// Mirrors #img-stats-bar + #bulk-img-modal + loadImageStats + doBulkImageUpload
// (menu.js:2330-2412). Per-item image uploads happen inside ItemFormModal —
// this section is only for the bulk-match-by-filename workflow + coverage bar.
const MAX_FILES = 20;
...
export default function ImageManagementSection() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const data = await bulkUploadImages(files);
      setResults({
      ...
      showToast(err?.response?.data?.error || err.message || 'Bulk upload failed', 'error');
    } finally {
      setUploading(false);
    }
  };
