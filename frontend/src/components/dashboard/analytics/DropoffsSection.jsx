...
import { getDropoffs, recoverDropoff } from '../../../api/restaurant.js';
import { useToast } from '../../../components/Toast.jsx';

// Mirrors loadDropoffAnalytics() + renderDropoffCards/FunnelBars/List()
// + sendRecovery() in legacy analytics.js:268-391. Dropoff endpoint uses
// ISO from/to derived from days; we map preset → days.
...
function lastActiveLabel(hours) {
...
function FunnelBars({ funnel }) {
  ...
  funnel.forEach((f, i) => {
    ...
    if (i < funnel.length - 1) {
      const drop = funnel[i].count - funnel[i + 1].count;
      const dropPct = funnel[i].count ? Math.round((drop / funnel[i].count) * 100) : 0;
      if (drop > 0) {
        rows.push(
...
function DropoffRow({ item, onRecovered }) {
  const { showToast } = useToast();
  const [confirming, setConfirming] = useState(false);
  const [busy, setBusy] = useState(false);
  const [recovered, setRecovered] = useState(false);

  const canRecover = (item.stage === 'cart' || item.stage === 'payment_pending') && item.hours_since_activity <= 48;
  ...
  const handleRecover = async () => {
    ...
    setBusy(true);
    try {
      const result = await recoverDropoff(item.conversation_id);
      if (result?.success) {
        setRecovered(true);
      ...
      showToast(err?.response?.data?.error || err.message || 'Failed to send recovery message', 'error');
    } finally {
      setBusy(false);
      setConfirming(false);
    }
