...
import { getPeakHours } from '../../../api/restaurant.js';

// Mirrors anLoadPeakHours() in legacy analytics.js:163-208.
...
export default function PeakHoursSection({ dateRange }) {
  const period = dateRange.preset;
  const { data, loading, error, refetch } = useAnalyticsFetch(
    useCallback(() => getPeakHours({ period }), [period]),
    [period],
  );
