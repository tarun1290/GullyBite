...
import CampaignCreateForm from './CampaignCreateForm.jsx';
import CampaignRoiCard from './CampaignRoiCard.jsx';

// Mirrors loadCampaigns() + action handlers in legacy restaurant.js:574-817.
// Status → legacy badge-class map lives in STATUS_BADGE; row actions are
// gated by status per the legacy row template (lines 625-628).
const STATUS_BADGE = {
  draft: 'bd',
  scheduled: 'bb',
  sending: 'bb',
...
function CampaignActions({ campaign, onChanged }) {
  const { showToast } = useToast();
  const [confirm, setConfirm] = useState(null); // 'send' | 'pause' | 'resume' | 'delete'
  ...
  const run = async (kind) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.error || err.message || `${kind} failed`, 'error');
    } finally {
      setBusy(false);
      setConfirm(null);
    }
...
export default function CampaignsSection() {
  const campaignsQ = useAnalyticsFetch(
    useCallback(() => getCampaigns(), []),
    [],
  );
  const usageQ = useAnalyticsFetch(
    useCallback(() => getCampaignDailyUsage(), []),
    [],
  );

  const campaigns = Array.isArray(campaignsQ.data) ? campaignsQ.data : [];
  const usage = usageQ.data;
  const atCap = !!usage && Number(usage.daily_cap) > 0 && Number(usage.sent_today) >= Number(usage.daily_cap);
