...
// Mirrors loadCoupons/createCoupon/toggleCoupon/deleteCoupon in legacy
// restaurant.js:485-569. Create form is inline (no modal in legacy). Delete
// uses a two-click confirm instead of window.confirm — matches the pattern
// established in Orders/Analytics/Messages.
const EMPTY_FORM = {
  code: '',
  type: 'percent',
  value: '',
...
function discountLabel(c) {
  if (c.discount_type === 'percent') {
    const pct = `${parseFloat(c.discount_value).toFixed(0)}%`;
    return c.max_discount_rs
      ? `${pct} (max ₹${parseFloat(c.max_discount_rs).toFixed(0)})`
      : pct;
  }
  return `₹${parseFloat(c.discount_value).toFixed(0)} flat`;
}
...
function CouponRow({ coupon, onChanged }) {
  ...
  return (
    <tr style={{ borderBottom: '1px solid var(--rim)' }}>
      ...
      <td style={{ padding: '.65rem 1rem' }}>₹{parseFloat(coupon.min_order_rs || 0).toFixed(0)}</td>
