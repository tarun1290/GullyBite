...
import { updateRestaurantProfile, updateRestaurantSlug } from '../../../api/restaurant.js';

// Mirrors loadProfile() + doSaveProfile() + doUpdateSlug() in legacy
// settings.js:534-728. Keeps the view/edit toggle pattern (see
// toggleSettingsEdit at settings.js:2331). Uses useRestaurant() — no refetch.

const TYPE_LABELS = { both: 'Veg & Non-Veg', veg: 'Pure Veg', non_veg: 'Non-Veg Only' };

function ViewRow({ label, value, mono, badge }) {
...
function VerifBadge({ verified, hasValue }) {
  if (verified) {
    return (
      <span style={{
...
export default function BusinessInfoSection() {
  ...
  useEffect(() => {
    if (restaurant) {
      setForm(buildForm(restaurant));
      setSlug(restaurant.store_slug || '');
      setStoreUrl(restaurant.store_url || '');
    }
  }, [restaurant]);
  ...
  const slugPlaceholder = useMemo(() => {
    const source = restaurant?.business_name || '';
  ...
  const handleSaveSlug = async () => {
    ...
    setSavingSlug(true);
    try {
      const r = await updateRestaurantSlug(clean);
      if (r?.store_url) setStoreUrl(r.store_url);
      showToast('Store URL updated!', 'success');
      await refetch();
    } catch (err) {
      showToast(err?.response?.data?.error || err.message || 'Failed to update slug', 'error');
    } finally {
      setSavingSlug(false);
    }
  };
      ...
      <div className="cb">
        {!editing ? (
          <div>
            ...
            <ViewRow label="Type" value={TYPE_LABELS[r.restaurant_type] || r.restaurant_type} />
            {r.logo_url && (
              <ViewRow
                label="Logo"
