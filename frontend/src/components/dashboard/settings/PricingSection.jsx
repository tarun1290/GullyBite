...
// Mirrors loadProfile()'s pricing block + doSaveChargeConfig() in legacy
// settings.js:561-569 + 756-770. Same view/edit toggle pattern.
...
export default function PricingSection() {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateRestaurantProfile({
        ...
        deliveryFeeCustomerPct: parseInt(form.deliveryFeeCustomerPct, 10) || 0,
        packagingChargeRs: parseFloat(form.packagingChargeRs) || 0,
        packagingGstPct: parseFloat(form.packagingGstPct),
      });
  ...
  const delPct = r.delivery_fee_customer_pct != null ? r.delivery_fee_customer_pct : 100;
  const pkg = r.packaging_charge_rs || 0;
  const pkgGst = r.packaging_gst_pct != null ? r.packaging_gst_pct : 18;

  const hintPct = Math.min(100, Math.max(0, parseInt(form.deliveryFeeCustomerPct, 10) || 0));
