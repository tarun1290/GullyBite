...
import useMetaOAuth from '../../hooks/useMetaOAuth.js';
...
export default function PgConnect({ onLogout, showPage, brandNameHint }) {
  ...
  const resolveStoreUrl = useCallback(async () => {
    ...
    if (token) {
      ...
      } catch (_) { /* network error — fall through to preview */ }
    }
  ...
  const onMetaSuccess = useCallback(async () => {
    try {
      ...
      toastRef.current(err?.userMessage || err?.message || 'Could not refresh account', 'error');
    }
  }, [login, showPage, navigate]);

  const onMetaError = useCallback((msg) => {
    if (!msg) return;
