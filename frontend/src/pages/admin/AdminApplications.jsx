...
// Mirrors admin.html loadApplications (2237-2366): approval queue for
// restaurant signups with GST/FSSAI compliance checks and approve/reject
// actions. Replaces the legacy global modal with an inline notes panel
// anchored at the clicked row.

const TYPE_LABEL = { veg: 'Pure Veg', non_veg: 'Non-Veg', both: 'Veg & Non-Veg' };
...
export default function AdminApplications() {
  ...
  const [pending, setPending] = useState(null); // { id, action: 'approve'|'reject', name }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErr(e?.response?.data?.error || e?.message || 'Failed to load applications');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const doVerify = async (r, type) => {
    ...
    try {
      ...
      showToast(e?.response?.data?.error || e?.message || 'Verify failed', 'error');
    } finally {
      setRowBusy(null);
    }
  };
