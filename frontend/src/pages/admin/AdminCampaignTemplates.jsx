...
// Admin CRUD for the campaign template library (campaign_templates
// collection). Distinct from AdminTemplates.jsx, which caches Meta's
// WABA template list; this page manages the curated catalog that
// restaurants pick from when composing campaigns.
...
const APPROVAL_STATUSES = ['pending', 'approved', 'rejected', 'paused'];
const VARIABLE_SOURCES = ['auto', 'restaurant_input', 'customer_data', 'system'];
...
function renderBodyPreview(body, variables) {
  if (!body) return '';
  return String(body).replace(/\{\{\s*(\w+)\s*\}\}/g, (_, name) => {
...
export default function AdminCampaignTemplates() {
  ...
  const [editing, setEditing] = useState(null); // template doc being edited, null when closed
  const [form, setForm] = useState(emptyForm());
  const [formErrors, setFormErrors] = useState({});
  const [saving, setSaving] = useState(false);
  const [rejectReasonByTid, setRejectReasonByTid] = useState({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await getAdminCampaignTemplates(params);
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      showToast(err?.response?.data?.error || err.message || 'Failed to load templates', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const setApproval = async (doc, status) => {
    try {
      const body = { status };
      if (status === 'rejected') {
        ...
        >
          <div className="card" style={{ width: 'min(960px, 100%)', marginBottom: '2rem' }}>
            <div className="ch" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3>{editing._new ? 'New Campaign Template' : `Edit: ${editing.template_id}`}</h3>
            ...
            <div className="cb" style={{ display: 'grid', gap: '.9rem', gridTemplateColumns: '1fr 1fr' }}>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '.5rem', justifyContent: 'flex-end' }}>
                ...
                <button className="btn-p" onClick={submit} disabled={saving}>
                  {saving ? 'Saving…' : (editing._new ? 'Create' : 'Save changes')}
                </button>
              </div>
            </div>
          </div>
