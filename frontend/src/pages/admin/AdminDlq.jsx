...
import { getDlqStats, getDlq, retryDlq, dismissDlq, getAdminLog } from '../../api/admin.js';

// Mirrors admin.html loadDlq/loadDlqStats/loadDlqTable/dlqRetry/dlqDismiss (2461-2546).
// 4-card stats strip, source filter, 7-col table with expandable error history,
// Retry/Dismiss/View actions (two-click confirm on Dismiss), offset pagination.
...
export default function AdminDlq() {
  ...
  const loadStats = useCallback(async () => {
    try {
      const d = await getDlqStats();
      setStats(d);
      setStatsErr(null);
    } catch (e) {
      setStatsErr(e?.response?.data?.error || e?.message || 'Failed to load DLQ stats');
    }
  }, []);

  const loadTable = useCallback(async () => {
    setLoading(true);
    const params = { limit: DLQ_LIMIT, offset };
    if (source) params.source = source;
    try {
      const d = await getDlq(params);
      setRows(d?.entries || []);
      setTotal(d?.total || 0);
      setErr(null);
    } catch (e) {
      setRows([]);
      setErr(e?.response?.data?.error || e?.message || 'Failed to load DLQ');
    } finally {
      setLoading(false);
    }
  }, [offset, source]);
  ...
  const doRetry = async (id) => {
    ...
    try {
      ...
      showToast(e?.response?.data?.error || e?.message || 'Retry failed', 'err');
    } finally {
      setBusy(null);
    }
  };
  ...
  const doDismiss = async (id) => {
    ...
    try {
      await dismissDlq(id);
      showToast('Entry dismissed', 'ok');
      setConfirmDismiss(null);
      await Promise.all([loadStats(), loadTable()]);
    } catch (e) {
      showToast(e?.response?.data?.error || e?.message || 'Dismiss failed', 'err');
    } finally {
      setBusy(null);
    }
  };
  ...
  const openDetail = async (id) => {
    ...
    setDetailErr(null);
    try {
      const d = await getAdminLog(id);
      setDetail(d);
    } catch (e) {
      setDetailErr(e?.response?.data?.error || e?.message || 'Failed to load entry');
    } finally {
