...
import { getAdminLogs, getAdminLog } from '../../api/admin.js';

// Mirrors admin.html loadLogs/openLogModal (2373-2455).
// Webhook/event log viewer with source/processed/has_error/event/date filters,
// offset pagination (limit 50), and a JSON payload detail modal.
...
export default function AdminLogs() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const params = { limit: LOGS_LIMIT, offset };
    ...
    try {
      const d = await getAdminLogs(params);
      setRows(d?.logs || []);
      setTotal(d?.total || 0);
      setErr(null);
    } catch (e) {
  ...
  const openDetail = async (id) => {
    ...
    try {
      ...
      setDetailErr(e?.response?.data?.error || e?.message || 'Failed to load log');
    } finally {
