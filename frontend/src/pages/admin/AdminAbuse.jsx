...
// Mirrors admin.html loadAbuse/loadAbuseStats/loadBlockedPhones/
// doBlockPhone/unblockPhone (2595-2728). 3 stat cards, top-rate-limited
// table with quick-block action, blocked-phones table with unblock, block modal.
...
export default function AdminAbuse() {
  ...
  const [blockedErr, setBlockedErr] = useState(null);

  const [modalOpen, setModalOpen] = useState(false);
  const [fPhone, setFPhone] = useState('');
  const [fReason, setFReason] = useState('');
  const [fDuration, setFDuration] = useState('24');
  ...
  const loadStats = useCallback(async () => {
    try {
      const d = await getRateLimitStats();
      setStats(d);
  ...
  const loadBlocked = useCallback(async () => {
    setLoading(true);
    try {
      const d = await getBlockedPhones();
      setBlocked(Array.isArray(d) ? d : []);
      setBlockedErr(null);
    } catch (e) {
      setBlocked([]);
      setBlockedErr(e?.response?.data?.error || e?.message || 'Failed to load blocked phones');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const submitBlock = async () => {
    ...
    try {
      ...
      showToast('Phone blocked', 'ok');
      closeModal();
      await Promise.all([loadStats(), loadBlocked()]);
    } catch (e) {
      showToast(e?.response?.data?.error || e?.message || 'Block failed', 'err');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const doUnblock = async (id) => {
    ...
    try {
      ...
      showToast('Phone unblocked', 'ok');
      setConfirmUnblock(null);
      await Promise.all([loadStats(), loadBlocked()]);
    } catch (e) {
      showToast(e?.response?.data?.error || e?.message || 'Unblock failed', 'err');
    } finally {
      setBusyId(null);
    }
  };
