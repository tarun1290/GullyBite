...
function _formatHoursSummary(b) {
  ...
  const openDays = _dayNames.filter(d => !b.operating_hours[d]?.is_closed);
  ...
  if (allSame) {
    ...
    return `${timeStr} (${closedCount} day${closedCount > 1 ? 's' : ''} closed)`;
  }
  return 'Custom schedule';
}

async function loadOperatingHours() {
  const container = document.getElementById('op-hours-container');
  if (!container) return;
  try {
    const brList = branches && branches.length ? branches : await api('/api/restaurant/branches');
    if (!brList.length) {
      container.innerHTML = '<div class="empty"><div class="ei">🏪</div><h3>No branches yet</h3><p>Add a branch from the Menu tab first.</p></div>';
      return;
    }
    container.innerHTML = brList.map(b => `
      <div class="card" style="margin-bottom:1rem;padding:1rem 1.2rem">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.6rem">
          <div>
            <strong style="font-size:.92rem">${b.name}</strong>
            <span style="font-size:.78rem;color:var(--dim);margin-left:.6rem">${_formatHoursSummary(b)}</span>
          </div>
          <button class="btn-g btn-sm" onclick="openHoursEditor('${b.id}')">Edit Hours</button>
        </div>
        <div id="hours-editor-${b.id}" style="display:none;margin-top:.5rem;padding:.8rem;background:var(--ink2);border:1px solid var(--bdr);border-radius:8px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.6rem">
    ...
    container.innerHTML = `<div class="empty"><div class="ei">❌</div><h3>${e.message}</h3></div>`;
  }
}
...
window.loadOperatingHours = loadOperatingHours;
