...
async function loadProfile() {
  try {
    ...
    // Hydrate the homepage banner button from the canonical constant
    // (eliminates drift if the label is ever changed in shared.js).
    const bannerBtn = document.getElementById('banner-connect-btn');
    if (bannerBtn && !bannerBtn.disabled) bannerBtn.innerHTML = waConnectBtnHTML(false);
  } catch (_) {}
}
...
// Tracks whether the Settings card is showing the "Reconnect" or "Connect" variant.
// Set by loadProfile() based on rest.meta_user_id + rest.whatsapp_connected.
// Used by _resetConnectBtns() to restore the correct label after a loading state.
let _waCardIsReconnect = false;

function _resetConnectBtns() {
  // Banner is always "Connect" variant — it only appears when nothing is connected.
  _setBtnLoading('banner-connect-btn', false, waConnectBtnHTML(false));
  // Settings card may be "Reconnect" if a broken connection exists.
  _setBtnLoading('wa-reconnect-btn', false, waConnectBtnHTML(_waCardIsReconnect));
}
