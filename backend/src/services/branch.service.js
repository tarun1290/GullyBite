...
// MONTHLY_FEE_PAISE import removed — branch onboarding is admin-gated;
// no upfront Razorpay subscription order is created at branch creation.
...
async function createBranch(input) {
  ...
  const branch = {
    ...
    is_open: true,
    accepts_orders: true,
    // Admin-gated onboarding. Set explicitly here rather than relying on
    // schema metadata defaults — the schema is descriptive, not a runtime
    // validator, so the writer is the source of truth. Branch is created
    // 'pending_approval'; an admin flips it to 'active' via
    // POST /api/admin/branches/:branchId/approve (or bulk-approve).
    // paid_through_date is intentionally NOT initialized at creation.
  ...
  log.info({ branchId: branch._id, restaurantId: branch.restaurant_id, city: branch.city }, 'branch created (pending_approval)');
  return { branch };
}
